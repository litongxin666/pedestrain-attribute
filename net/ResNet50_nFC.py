...
class ClassBlock(nn.Module):
    def __init__(self, input_dim, num_bottleneck = 256):
...
class ResNet50_nFC(nn.Module):
    def __init__(self, class_num):
        ...
        model_ft.fc = nn.Linear(fc_features, 1024)
    ...
    def forward(self, x):
        ...
        for c in range(self.class_num):
            if c == 0:
                pred = self.__getattr__('class_%d' % c)(x_inter)
            else:
                pred = torch.cat((pred, self.__getattr__('class_%d' % c)(x_inter) ), dim=1)
        return pred,x_inter
